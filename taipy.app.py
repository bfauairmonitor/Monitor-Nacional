...
with col_inf_2: 
    try:
        ...
        fig4.update_layout(title=dict(text="BASE MONETARIA BCV", font=dict(color="white")), paper_bgcolor='Black', plot_bgcolor='Black', height=ALT_INF, margin=dict(l=5, r=5, t=30, b=40), xaxis=dict(tickfont=dict(color="white", size=15)), yaxis=dict(showticklabels=False, range=[montos4.min()*-0.4, montos4.max()*1.4]), showlegend=False)
...
with col_inf_3: 
    try:
        ...
        fig5.update_layout(title=dict(text="LIQUIDÉZ MONETARIA BCV", font=dict(color="white")), paper_bgcolor='Black', plot_bgcolor='Black', height=ALT_INF, margin=dict(l=5, r=10, t=35, b=40), xaxis=dict(tickfont=dict(color="white", size=15)), yaxis=dict(showticklabels=False, range=[montos5.min()*-0.4, montos5.max()*1.4]), showlegend=False)
