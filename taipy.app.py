...
# ==========================================
# 3. ESTILOS CSS (CORREGIDO PARA PYTHON)
# ==========================================
st.markdown(f"""
<style>
...
with col_sup_izq: # G1: TASA OVERNIGHT DIARIA
    try:
        ...
        fig1.update_layout(
            title=dict(text="Tasa Overnight Diaria", font=dict(color="white", size=20)),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=ALT_SUP,
...
with col_sup_der: # G2: RESERVAS EXCEDENTARIAS
    try:
        ...
        fig2 = go.Figure(go.Bar(
            ...
            text=[f"{v/1000:,.3f}MM" for v in df2.iloc[:, 1]], textposition='outside',
            marker_color=C_AZUL, textfont=dict(size=16, color="white")
        ))
        fig2.update_layout(
            title=dict(text="Reservas Bancarias Excedentarias", font=dict(color="white", size=20)),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=ALT_SUP,
            margin=dict(l=10, r=10, t=50, b=40),
            xaxis=dict(tickangle=-30, tickfont=dict(color="white", size=15)),
            yaxis=dict(gridcolor='#222222', tickfont=dict(color="white"))
...
with col_inf_1: # G3: TASA OVERNIGHT MENSUAL
    try:
        ...
        fig3.update_layout(
            title=dict(text="Tasa Overnight Mensual", font=dict(color="white", size=18)),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=ALT_INF,
...
with col_inf_2: # G4: BASE MONETARIA
    try:
        ...
        fig4 = go.Figure()
        fig4.add_trace(go.Bar(x=fechas4, y=montos4, text=[f"{v:,.1f}MM" for v in montos4], textposition='outside', marker_color='#4D79FF', textfont=dict(color="white", size=15)))
        escala4 = montos4.max() / (var4.abs().max() if var4.abs().max() != 0 else 1)
        fig4.add_trace(go.Scatter(x=fechas4, y=var4 * escala4 * 0.7, mode='lines+markers+text', text=[f"{v:.2f}%" for v in var4], textposition="top center", line=dict(color=C_NARANJA, width=3, shape='spline'), marker=dict(size=8, color='white'), textfont=dict(color=C_NARANJA, size=15)))
        
        fig4.update_layout(
            title=dict(text="Base Monetaria", font=dict(color="white", size=18)),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=ALT_INF,
            margin=dict(l=5, r=5, t=50, b=40),
            xaxis=dict(tickfont=dict(color="white", size=15)),
...
with col_inf_3: # G5: LIQUIDEZ MONETARIA
    try:
        ...
        fig5 = go.Figure()
        fig5.add_trace(go.Bar(x=fechas5, y=montos5, text=[f"{int(v):,}MM" for v in montos5], textposition='outside', marker_color='#483D8B', textfont=dict(color="white", size=15)))
        escala5 = montos5.max() / (var5.abs().max() if var5.abs().max() != 0 else 1)
        fig5.add_trace(go.Scatter(x=fechas5, y=var5 * escala5 * 0.7, mode='lines+markers+text', text=[f"{v:.2f}%" for v in var5], textposition="top center", line=dict(color=C_NARANJA, width=3, shape='spline'), marker=dict(size=8, color='white'), textfont=dict(color=C_NARANJA, size=15)))

        fig5.update_layout(
            title=dict(text="Liquidez Monetaria", font=dict(color="white", size=18)),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=ALT_INF,
            margin=dict(l=5, r=10, t=50, b=40),
            xaxis=dict(tickfont=dict(color="white", size=15)),
...
with col_inf_4: # G6: RESERVAS INTERNACIONALES $
    try:
        ...
        fig6 = go.Figure()
        fig6.add_trace(go.Bar(x=fechas6, y=montos6, text=[f"{int(v):,}MM" for v in montos6], textposition='outside', marker_color='#191970', textfont=dict(color="white", size=15)))
        escala6 = montos6.max() / (var6.abs().max() if var6.abs().max() != 0 else 1)
        fig6.add_trace(go.Scatter(x=fechas6, y=var6 * escala6 * 0.7, mode='lines+markers+text', text=[f"{v:.2f}%" for v in var6], textposition="top center", line=dict(color=C_NARANJA, width=3, shape='spline'), marker=dict(size=8, color='white'), textfont=dict(color=C_NARANJA, size=16)))

        fig6.update_layout(
            title=dict(text="Reservas Internacionales $", font=dict(color="white", size=18)),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=ALT_INF,
            margin=dict(l=5, r=10, t=50, b=40),
